...
def get_bigquery_client():
    ...
    try:
        ...
        if "gcp_service_account" not in st.secrets:
            ...
            st.info("""
            Configure as credenciais no Streamlit Secrets:
            1. Localmente: Crie um arquivo `.streamlit/secrets.toml`
            2. Na nuvem: Adicione no painel de Secrets do Streamlit Community Cloud
            ...
            client_x509_cert_url = "https://www.googleapis.com/robot/v1/metadata/x509/seu-email%40project.iam.gserviceaccount.com"
...
if st.session_state.df_completo.empty:
    st.info("👈 Configure os filtros e clique em 'Carregar TODOS os Dados' para começar")
    
    # Seção de configuração inicial
    with st.expander("⚙️ Configuração Inicial", expanded=True):
        st.write("""
        ### Para usar este aplicativo, você precisa:
        
        1. **Configurar credenciais do Google Cloud:**
           - Crie um arquivo `.streamlit/secrets.toml` localmente
           - Ou configure no painel de Secrets do Streamlit Community Cloud
        
        2. **Formato do arquivo secrets.toml:**
        ```toml
        [gcp_service_account]
        type = "service_account"
        project_id = "macfor-media-flow"
        private_key_id = "929c3d6c60e8c0865f4e969b02701b31087829c3"
        private_key = \"\"\"-----BEGIN PRIVATE KEY-----\\nMIIEvg...\\n-----END PRIVATE KEY-----\"\"\"
        client_email = "bigquery@macfor-media-flow.iam.gserviceaccount.com"
        client_id = "116209385893678517861"
        auth_uri = "https://accounts.google.com/o/oauth2/auth"
        token_uri = "https://oauth2.googleapis.com/token"
        auth_provider_x509_cert_url = "https://www.googleapis.com/oauth2/v1/certs"
        client_x509_cert_url = "https://www.googleapis.com/robot/v1/metadata/x509/bigquery%40macfor-media-flow.iam.gserviceaccount.com"
        universe_domain = "googleapis.com"
        ```
        
        3. **Teste a conexão** no painel lateral antes de carregar dados
        """)
        
        # Botão para criar template de secrets.toml
        if st.button("📋 Gerar Template do secrets.toml"):
...
"""
            ...
            st.download_button(
                ...
                data=template,
                file_name="secrets_template.toml",
                mime="text/plain"
            )
